...
// Day1 '좋은 입력' 자가검증 게이트 — 스펙(마크다운)이 "검증 가능한 형태"를 갖췄는지 결정론적으로 점검.
// 의존성 0. 사용: node scripts/spec-check.mjs [경로]  (기본: spec.md → FEATURE-SPEC.md → templates/feature-spec-card.md)
...
if (!file) {
  ...
  console.error('  사용: node scripts/spec-check.mjs <스펙.md>   (또는 FEATURE-SPEC.md 를 만드세요)');
  process.exit(2);
}
